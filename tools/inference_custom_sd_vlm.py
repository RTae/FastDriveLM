...
if TYPE_CHECKING:
    from custom_sd_vlm import CustomSDVLM
...
def _resolve_model_and_lora(model_or_adapter: str) -> tuple[str, str | None]:
    ...
    if adapter_config_path.exists():
        ...
        if not base_model:
            raise ValueError(
                f"LoRA adapter is missing base_model_name_or_path: {adapter_config_path}"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="DriveLM inference with custom SSD VLM runtime using SpargeAttn prefill and SageAttention decode",
    )
    ...
    parser.add_argument("--output", default="outputs/qwen3vl/infer_results_custom_sd_vlm.json", help="Output JSON path")
    ...
    parser.add_argument("--enforce-eager", action="store_true", help="Accepted for CLI compatibility; this backend currently runs in eager mode")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose SSD runtime logging")
...
def main() -> None:
    args = parse_args()
    from custom_sd_vlm import SamplingParams
    ...
    results = []
    per_sample_metrics = []
    for sample in tqdm(dataset, desc="Custom SD VLM Inference", dynamic_ncols=True):
        prompt, images, question, sample_id = _sample_to_prompt(sample)
        first_token_time = [None]

        def on_tokens(_seq_id, new_ids):
        ...
        outputs, metrics = engine.generate(
            [prompt],
            [SamplingParams(temperature=args.temperature, max_new_tokens=args.max_new_tokens)],
            use_tqdm=False,
        ...
        ttft_sec = (first_token_time[0] - start_time) if first_token_time[0] is not None else 0.0
        decode_time_sec = end_time - (first_token_time[0] or start_time)
